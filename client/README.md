...
- Dashboard analytics scaffold (`src/layouts/DashboardLayout.tsx`) with placeholder KPI, distribution, and recent-incident widgets that call `/api/dashboard/*` via typed hooks (`useDashboardLast24HoursKpi`, `useDashboardTypeDistribution`, `useDashboardSeverityDistribution`, `useDashboardDailyTrend`, `useDashboardRecentIncidents`) while sharing the same filter state as the incidents table
