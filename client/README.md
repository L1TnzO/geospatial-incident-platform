...
- Strategic analytics layout (`src/layouts/StrategicLayout.tsx`) that stitches together a month-over-month trend chart with timeframe toggles and export controls, a type-specific trend explorer with moving-average windows that synchronizes the global type filter, quarterly comparison, hotspot summary, response readiness, and priority score cards backed by the strategic hooks, auto-refreshes every five minutes, exposes per-card retry controls, and keeps a small roadmap section for upcoming executive widgets
...
`src/styles/strategic/strategic.scss` scopes the strategic analytics shell, cards, monthly trend chart partial (`_monthly-trend.scss`), the type explorer styles (`_type-explorer.scss`), and placeholder widgets.
