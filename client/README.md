...
- Dashboard analytics scaffold (`src/layouts/DashboardLayout.tsx`) with an interactive **Incidents (Last 24h)** KPI card (trend arrow, signed delta, percentage change, in-card refresh) plus distribution and recent-incident widgets that call `/api/dashboard/*` via typed hooks (`useDashboardLast24HoursKpi`, `useDashboardTypeDistribution`, `useDashboardSeverityDistribution`, `useDashboardDailyTrend`, `useDashboardRecentIncidents`) while sharing the same filter state as the incidents table
