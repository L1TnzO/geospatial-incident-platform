...
- Strategic analytics layout (`src/layouts/StrategicLayout.tsx`) that stitches together a month-over-month trend chart with timeframe toggles and export controls, a quarter-over-quarter comparison chart with grouped bars, delta annotations, and CSV/PNG exports, Leaflet-powered hotspot, station coverage, and response-time overlay cards (resolution/intensity controls, per-station toggles, grouping + threshold sliders, legends, refresh/cancel actions) backed by the `/api/strategic/hotspots`, `/api/strategic/coverage-buffers`, and `/api/strategic/response-metrics` endpoints, plus a synchronized hotspot summary card, a type-specific trend explorer with moving-average windows that synchronizes the global type filter, response readiness, and priority score cards backed by the strategic hooks, auto-refreshes every five minutes, exposes per-card retry/cancel controls, and keeps a small roadmap section for upcoming executive widgets
- Strategic analytics foundation (`src/services/strategicAnalyticsService.ts` + hooks under `src/hooks/useStrategic*.ts`) that wires the new Phase 6 `/api/strategic/*` endpoints into reusable React hooks. The services mirror dashboard query-string helpers, accept shared filter context, expose manual/TTL-based refresh controls, and surface typed results (monthly/quarterly trends, incident type timelines, hotspot grids, response metrics, priority scores) ready for the `/strategic` route and future executive dashboards. The monthly trends hook now tracks cached windows per timeframe (6/12/24 months by default) and exposes CSV/PNG exports for executive reporting
...
`src/styles/strategic/strategic.scss` scopes the strategic analytics shell, cards, monthly trend chart partial (`_monthly-trend.scss`), quarter comparison styles (`_quarter-comparison.scss`), the type explorer styles (`_type-explorer.scss`), and placeholder widgets.
