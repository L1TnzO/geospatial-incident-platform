...
- Strategic analytics layout (`src/layouts/StrategicLayout.tsx`) that stitches together a month-over-month trend chart with timeframe toggles and export controls, quarterly comparison, type timeline, hotspot summary, response readiness, and priority score cards backed by the strategic hooks, auto-refreshes every five minutes, exposes per-card retry controls, and keeps a small roadmap section for upcoming executive widgets
...
`src/styles/strategic/strategic.scss` scopes the strategic analytics shell, cards, monthly trend chart partial (`_monthly-trend.scss`), and placeholder widgets.
