...
- Strategic analytics layout (`src/layouts/StrategicLayout.tsx`) that stitches together a month-over-month trend chart with timeframe toggles and export controls, a quarter-over-quarter comparison chart with grouped bars, delta annotations, timeframe toggles, and CSV/PNG exports, a type-specific trend explorer with moving-average windows that synchronizes the global type filter, hotspot summary, response readiness, and priority score cards backed by the strategic hooks, auto-refreshes every five minutes, exposes per-card retry controls, and keeps a small roadmap section for upcoming executive widgets
