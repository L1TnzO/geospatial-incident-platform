...
async function verify() {
  ...
  if (process.env.DATABASE_URL) {
    console.log(
      `Original DATABASE_URL found (masked): ${process.env.DATABASE_URL.replace(/:([^:@]+)@/, ':****@')}`
    );

    // Replace container hostnames with localhost
    // Replace container hostnames with localhost and map internal port 5432 to host port 5434
    process.env.DATABASE_URL = process.env.DATABASE_URL.replace('@db:5432', '@localhost:5434')
      .replace('@db:', '@localhost:') // Fallback if no port specified
      .replace('@gip-postgis:', '@localhost:')
      .replace('@postgres:', '@localhost:');

    console.log(
      `Adjusted DATABASE_URL for host (masked): ${process.env.DATABASE_URL.replace(/:([^:@]+)@/, ':****@')}`
    );
  } else {
